...
for lambda_l1 in lambdas:
    ...
    reg = Lasso(alpha=lambda_l1, max_iter=10000) # alpha is our regularisation param
...
for i in range(len(ho_mse_per_lambda)):
    if ho_mse_per_lambda[i] == min(ho_mse_per_lambda): # if minimum E_ho
        ...
        reg = Lasso(alpha=lambdas[i], max_iter=10000) # alpha is our regularisation param
        reg.fit(X_train, y_train)
...
for trial in range(10):
    ...
    # train and fit the best regression model
    reg = Lasso(alpha=best_lambda, max_iter=10000) # alpha is our regularisation param
    reg.fit(X_train, y_train)
