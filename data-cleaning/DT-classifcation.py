...
DTClassif.fit(XTrainClassif, yTrainClassif)
# Use the model to predcit classes based on the test inputs
yPredClassif = DTClassif.predict(XTestClassif)
...
Y = DTClassif.predict(np.c_[x1.ravel(), x2.ravel()])
...
plt.title(f'DT Decision Regions for depth={depth}')
plt.show()
